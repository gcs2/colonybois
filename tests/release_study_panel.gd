...
func _draw() -> void:
	...
	var blocked: bool=stage in ["duplicate","no-food","no-energy","blocked"]
	...
	var title: String={"blocked":"Cannot release here","armed":"Choose ground to release","approach":"Moving into range","beam":"Releasing specimen","released":"Established on Morrow","duplicate":"Already established","no-food":"Food supply incomplete","no-energy":"Not enough energy","cancelled":"Release cancelled"}[stage]
	text(Vector2(1310,922),title,21,DARK)
	var detail: String={"blocked":"","armed":"1 specimen + 5 energy on success","approach":"No resources spent until release completes","beam":"1 specimen + 5 energy on success","released":"1 specimen released · 5 energy used","duplicate":"This species already occupies an ecological slot.","no-food":"T2 needs small, medium and large plants first.","no-energy":"Requires 5 energy; you have 4. Use a pack or dock.","cancelled":"Specimen kept · no energy spent"}[stage]
	if not refusal_detail.is_empty() and blocked: detail=refusal_detail
	if detail.length()>62:
		var split: int=detail.rfind(" ",62)
		text(Vector2(1310,951),detail.substr(0,split),18,DARK)
		text(Vector2(1310,976),detail.substr(split+1),18,DARK)
	else: text(Vector2(1310,951),detail,18,DARK)
	...
	for i: int in range(6):
		...
		elif stage=="cancelled" or slot.is_empty() or game.biosphere.SLOTS[i]!=slot[1]: text(rect.position+Vector2(27,32),"–",22)
		if stage!="cancelled" and not slot.is_empty() and game.biosphere.SLOTS[i]==slot[1]:
			corners(rect,accent)
			if stage=="released":
