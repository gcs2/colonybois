...
func run() -> void:
	...
	check(hud.palette_expanded and hud.toolbar[2].visible and hud.slot_labels.heat_ray.text == "1" and hud.slot_labels.warm.text == "9" and hud.slot_labels.seed.text == "Ctrl+1","Opening a category labels its own visible slots")
	press(scene,KEY_0)
	...
	hud.category_buttons.Environment.pressed.emit()
	press(scene,KEY_9)
	check(scene.tool == "warm" and hud.selected_tool == "warm" and scene.model.state == state,"Visible slot 9 selects the same thermal tool as clicking its icon")
