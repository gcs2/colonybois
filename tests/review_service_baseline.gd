...
const Study=preload("res://tests/service_study_panel.gd")
...
func run() -> void:
	...
	for stage: String in ["home-free","home-recharged","away-paid","unaffordable","energy-full","pack-sold-out","pack-locker-full","out-of-range","embargo","repair-locked","recharged","repair-purchased","repair-used"]:
		...
		if proposal:
			...
			study=Study.new(); study.game=game; study.scene=scene; study.stage=stage; study.selected=action if action!="recharge" else "pack"
			overlay.add_child(study)
		for resolution: Vector2i in [Vector2i(1920,1080),Vector2i(2560,1440)]:
			view.size=resolution
			for frame: int in range(5): await process_frame
			if stage=="repair-locked" and not proposal:
		...
		if proposal: records.back().merge({"proposed_selected":study.selected,"proposed_buy_reason":study.purchase_reason,"proposed_refuel_reason":study.recharge_reason,"proposed_price":study.shown_price})
		view.free()
	var file:=FileAccess.open(OUT+("/proposals.json" if proposal else "/baseline.json"),FileAccess.WRITE); file.store_string(JSON.stringify(records,"\t"))
	print("Service %s:26 captures; atomic refusals, free/paid recharge, separate pack purchase/use and immutable render verified." % ("proposals" if proposal else "baseline"))
	quit()
