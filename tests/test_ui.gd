...
func run() -> void:
	...
	check(scene.overview_globe.planet_definition.geography_seed == scene.sim.state.planets.s1p0.seed,"Strategic globe uses the full persistent planet seed")
	scene.overview_globe.set_recovery(0.5)
	check(scene.overview_globe.material.get_shader_parameter("recovery") == 0.5 and not scene.overview_globe.recovery_maps.is_empty(),"Climate project drives the shared globe renderer")
