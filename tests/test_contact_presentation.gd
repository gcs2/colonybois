...
func run() -> void:
	...
	check(market != null and market.entries.size() == 3,"Market exposes three pictorial goods")
	before = scene.campaign.snapshot()
	market.entries.water.pressed.emit()
	check(scene.campaign.snapshot() == before and scene.commodity_preview == "water","Selecting a commodity never mutates the campaign")
	check(not market.buy_button.disabled and market.sell_button.disabled,"Available stock can be bought while empty cargo cannot be sold")
	...
	check(market.selected_id == "water" and not market.sell_button.disabled,"Purchase retains selected good and enables selling actual cargo")
	var sell_price: int = scene.campaign.commerce.price(scene.model.state.planet_id,"water",false,scene.campaign)
	var sale_balance: float = scene.campaign.field.marks
	market.sell_button.pressed.emit()
	check(scene.campaign.commerce.quantity("water") == cargo_before and scene.campaign.field.marks == sale_balance+sell_price,"Sell button removes actual cargo and credits the current quote")
	market = scene.popup_body.find_child("CommodityShop",true,false)
