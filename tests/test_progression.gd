...
func run() -> void:
	...
	check(game.commerce.state.badges.size() == 12 and Recognition.points(game.commerce.state.badges) == 0,"Fresh campaign starts with eleven empty accomplishment families and no free recognition")
