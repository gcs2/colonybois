...
Read the [current story treatment](docs/STORY_CURRENT.md) and [complete project vision](docs/PROJECT_VISION.md). The campaign now begins in a modern city of approximately 120,000, with the Vanguard's rise leading into national and interstellar politics. The target is SimCity 4-like urban fidelity using aggregate population, jobs and services, not individually simulated citizens. The [city study](docs/CITY_CONCEPT.md) explores the revised scale. This opening, domestic nations and branching political abilities are design work; the playable build still begins with a small colony. The Marks currency display and historical tooltip are implemented; its ledger-origin history remains a proposed lore detail.
