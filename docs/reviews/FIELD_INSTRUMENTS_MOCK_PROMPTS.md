...
The user selected artifacts/field-instruments-review/01-surface-populated-v3.png as the Morrow surface design target; it is byte-identical to the image the user pinned. This supersedes its earlier provisional status. The user rejected all Codex-generated orbit-to-surface boards from this continuation. Do not iterate or implement from those boards. Put future missing-view requests in the [designer intake](../art/mock-requests/README.md), beginning with the [Morrow flight-view brief](../art/mock-requests/MORROW_ORBIT_APPROACH_SURFACE.md). The user has authorized a designer agent to generate three proposals for this gap; see the brief and shared artifact home at artifacts/designer-mocks/R01-V01-V04/. Prompt blocks below are historical records, not standing instructions to generate.
