...
Status: the exact Morrow surface, orbital, system and approach targets plus the supporting galaxy reference are accepted and tracked in [visual canon](../../art/visual-canon/README.md). This register covers additional view families and substates; many prompts remain proposals and do not imply those views are built or accepted. Earlier Codex-generated transition boards were rejected. The user requested concise prompts for a competent external designer/generator when a genuinely new mock is needed. Do not regenerate accepted images or call Codex imagegen for them unless the user asks. The task board and handoff own implementation status.

## Latest user decision — 25 September 2026

The user pinned the Morrow surface target and approved the exact v2 orbital gameplay, solar-system map and approach/arrival images on 25 September 2026; they also requested the earlier galaxy-scale reference. These five originals now live in the tracked [visual canon](../../art/visual-canon/README.md). This supersedes the earlier instruction to generate these particular views. The earlier Codex-generated transition boards remain rejected. Use the [designer intake](../art/mock-requests/README.md) and task-board-backed briefs only for genuinely missing views; prompts must cite the canon, source evidence and connected game context. Prompt blocks below preserve history and provenance; they do not authorize regeneration.

Historical 24 September direction: first aggregate actual Spore references, current captures and proposals. That comparison foundation and the 25 September canon selection followed later; the source transition and remaining view-state gaps are still open. Use larger physical tool artwork (about 40% more area, 18% linear) within compact controls rather than larger panel shells. Keep reference2's labels/units/layout out of unrelated scales; no parsecs on a surface chart. No foreground blur concealing gameplay. A galaxy target inside a 3 pc reach must actually lie inside its projected range. See VISUAL_FIDELITY_FEASIBILITY.md for renderer facts and production limitations.
