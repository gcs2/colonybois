...
Latest priority update: [Space-first direction](../direction/SPACE_FIRST_DIRECTION.md) supersedes the city-first sequence below. Cities now support exploration, logistics, administration and eventual fleet manpower. Preserve their existing prototype, but do not require a full municipal game before reaching space. Later first-person ship combat is desired; [the technical assessment](../research/FLIGHT_TECH_ASSESSMENT.md) proposes an early small feasibility test. [Diplomacy and the chronicle](../direction/DIPLOMACY_AND_CHRONICLE.md) define expressive alien contact, meaningful trade and persistent history. The older urban detail below remains deferred design material, not the immediate production order.
...
Use generated imagery for concept boards, portraits, icons and surface references. Build actual game geometry from reusable modular meshes. Images are not game-ready 3D assets. Review concept boards for scale and direction, then test camera, selection and readability in Godot. [The urban study](../reviews/CITY_CONCEPT.md) tests the revised scale; it is not an approved final art style or a gameplay screenshot.
...
See [Roadmap](../ROADMAP.md) for acceptance gates and [Architecture](../systems/ARCHITECTURE.md) for implementation boundaries. No reliable calendar estimate exists for the full vision yet. The scope should grow in response to observed play, not merely because a system sounds exciting.
