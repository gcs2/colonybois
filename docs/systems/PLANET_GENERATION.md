...
**Current gap:** The shared recipe now supplies orbital and surface sampling. Saved radial direction drives the scout across bounded macro regions, and the terrain/feature windows recenter while sparse stable-ID changes persist through save/reload, inactive-world snapshots, and legacy migration. A separate deterministic 64 m habitat query streams within a bounded 192 m radius without changing the existing macro-region IDs or opportunity placement; coarse terrain rebuilds only when the larger region changes. This provides repeatable fine features, not finished regional composition or full planetary traversal.

The integrated 1920×1080 mining capture combines the actual HUD and current Morrow surface runtime. Two independent visual reviews reject it against canon: nearby habitat is small and sparse, the ground is mostly flat brown with an empty horizon, and the scene lacks the canon’s ridges, water, readable rock forms, and life. The target label/card still feel detached even though the card no longer covers the scout. The UI is legible, but its materials and synthetic 0-Mark state do not match the target.

Latest focused checks on this integrated branch: test_planet_surface_runtime.gd passed 18 checks in 0.47 s; test_surface_exploration.gd passed 63 assertions in 38.36 s; test_flight_hud.gd passed 64 assertions in 20.28 s. The synthetic capture harness passed. No full suite, native input, exported build, player-fun review, or performance measurement was run. The local 1.2 km tangent-frame travel limit and single adjacent-region opportunity remain provisional; this is not whole-planet streaming.

Next, build the Morrow scene through shared geography and reusable rules for landform silhouette, relief, water/coast, geological forms, clustered habitat, opportunities, and chart marks. The fine query provides a stable local-detail layer, while the large-scale recipe and authored sites should determine composition. Preserve the current stable-ID/persistence contract and bound both active windows. Keep visual acceptance open until the actual scene approaches canon at multiple regions and scales.
