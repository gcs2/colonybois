...
Keep PlanetSurfaceRuntime as the one surface-query boundary. Before persistent deltas, split procedural draws by region and semantic kind and derive stable IDs from per-kind slots; the current single region RNG couples category positions and IDs to earlier category counts. Then make the saved radial direction authoritative for motion, recenter the bounded terrain/feature window, and save sparse deltas by stable feature/site ID. Do not widen travel by changing the radius again or add species/planets before the connected crossing works.
