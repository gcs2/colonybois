...
| Conqueror | Distinct accepted colony surrenders | 1 / 2 / 5 / 10 / 20 |
| Colonist | New landing hubs completed (excludes captures) | 1 / 2 / 5 / 10 / 20 |
...
The [earlier badge audit](../research/SPORE_BADGE_PARITY.md) and [reference manifest](../parity/reference_progression.json) remain the scope record. A fresh indexed [Master Badge](https://spore.fandom.com/wiki/Master_Badge) result confirmed ten promotion thresholds and fleet links; direct page retrieval was blocked. [Badge Point](https://spore.fandom.com/wiki/Badge_Point) and [StrategyWiki's badge listing](https://strategywiki.org/wiki/Spore/Badges) also blocked direct retrieval. Per-family points, disputed thresholds and exhaustive unlock edges remain unverified; the implementation's scenario tuning above is separate from those source claims.
...
The territorial checkpoint adds Conqueror from actual accepted surrenders; only its first two tiers fit the current three-colony cap. Snapshot v17 requires the new badge field; earlier snapshots migrate it to zero. Paid hull eligibility preserves current damage. See [TERRITORIAL_COMBAT.md](TERRITORIAL_COMBAT.md).
