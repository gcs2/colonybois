...
The reviewed Tripo GLB is one mesh primitive with one material; its internal mineral veins cannot be independently removed. The current procedural alternative makes each finite resource unit a separate basalt socket with three faceted crystal shards. First staged captures exposed a visibility bug: four opaque static rock meshes overlapped the mineable lodes and remained after those lodes were hidden. The current branch removes those occluding rocks. The 1080p harness now completes against the actual encounter scene and a real mining command, producing ignored `artifacts/visual-critic-surface-pass/lode-stages/morrow-seam-intact-4-of-4-1080.png`, `morrow-seam-3-of-4-1080.png` and `morrow-seam-0-of-4-1080.png`. Root inspection shows four shard clusters, then three, then an empty prospect ring; visible depletion is materially clearer in this capture. `tests/test_encounter.gd` (54 assertions) and `tests/test_expedition_session.gd` (48 assertions) pass. The old Tripo capture still shows little change after one cut; the large-GLB harness was not rerun. It previously parsed the source and generated review LODs, but packaged import, active runtime LOD selection, loading, memory and performance remain unverified. Current lodes remain a **provisional art candidate**: they are small in the full scene, the terrain/HUD remain below target, and no independent critic or native player accepted them. The next gate is same-state independent review, not another asset generation pass.
