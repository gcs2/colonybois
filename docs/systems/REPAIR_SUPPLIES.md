...
- **UI connection**: The dock service popup (`_build_service_contents`) displays the current hull state (`HULL %d / %d · DOCK REPAIR`) with a dynamic repair action button showing the exact missing hull and quoted Marks price (`"Repair hull to full (+%d) · %d Marks"`). When the hull is sound, it displays `"Hull sound"` and remains disabled with an explanatory tooltip. Clicking the button debits the exact tariff, heals the ship up to purchased capacity, triggers a confirmation toast and audio cue, saves campaign state, and refreshes the panel atomically.
...
The full suite passes with **1,010 assertions plus UI checks**, including 58 direct dock repair checks (`tests/test_dock_repair.gd`) verifying quotes, atomic refusals, multi-tier hull capacity scaling, local/foreign tariffs, embargoes, transit blocks, persistence roundtrips, real encounter UI button state, price display, click transactions, and invariant protection alongside 46 repair-supply checks. Renderer captures cover supply shop, inventory and palette at 1080p/1440p. Shortened repeated service text after inspecting the first render so both repair offers fit visibly at 1080p. Current art, audio, native input feel and difficulty balance remain unapproved; this is not AAA acceptance.
