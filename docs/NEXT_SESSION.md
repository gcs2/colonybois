...
**User-directed asset-pack exception:** the user then authorized a bounded asset pack and will handle Tripo Studio manually. Nine isolated prop concepts are in `art/concepts/tripo_asset_pack_v1/`; use the pack README for the precise Spore mapping and batch recipe. The orbital context image and exploratory post-human character are not Tripo prop inputs. Save manually exported, unreviewed GLBs under `assets/tripo_inbox/`; that directory's README explains promotion after review. Do not use computer control on Tripo. The generated Cthulian-descendant idea is exploratory only and does not reopen character modeling/cast production.

Character modeling and cast expansion remain **deferred** after the failed modeling demonstration. The user has since asked for one exploratory ocean-adapted post-human concept; it remains a story/art study, not a Tripo submission or new character pipeline. Existing Tavi concept art is a static portrait, not acting. Preserve the prototypes and read the [postmortem](reviews/MODELING_POSTMORTEM_2026-09-24.md). The overall space-game objective is unchanged.
