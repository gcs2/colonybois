...
Latest review evidence: seven deterministic inventory states now render at1080p/1440p using actual pack rules over static concept art. See the state table in [coverage review](reviews/VIEW_MOCK_COVERAGE.md). Corrected input-state styling is a design study, not a game HUD change. Next close populated category capacity and navigation state specifications; do not spend the next pass treating these three consumables as full possessions coverage.
