...
First complete the reference/current/mock coverage gate in [TASK_BOARD.md](TASK_BOARD.md), beginning with orbital/surface HUD. The user rejected continued implementation without comprehensive source fidelity and every-view mocks. [Coverage audit](reviews/VIEW_MOCK_COVERAGE.md): 17/61 sampled source workflows, 6/61 readable, zero fully verified interactions/presentation/audio; six base candidate files across 33 families, zero complete family state sets. These are evidence counts, not game completion. No further discretionary UI/feature production ahead of the gate. Existing builds remain available; character production remains deferred.

Latest review evidence: seven inventory states and twelve schematic navigation states render at1080p/1440p. Navigation now uses isolated actual campaign quotes/commands, including four additional refusal cases, and independent critique confirms legibility. These are review studies, not game HUD changes or final art. See [coverage](reviews/VIEW_MOCK_COVERAGE.md) and [interface contract](reviews/SPORE_INTERFACE_CONTRACT.md). Next address oversized contextual cards and missing world-context refusal states, then populated category capacity. Source input/motion/audio verification remains open. The defense-field quote/commit mismatch is now repaired in source;27 focused and45 interstellar checks pass. Build20260924-050435 includes this repair; exported-pack27-check regression and isolated flight startup pass.
