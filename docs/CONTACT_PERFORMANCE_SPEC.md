...
Our actual `artifacts/contact_consortium.png` buries choices in a scrolling generic inspector. `scripts/encounter.gd::_show_popup` removes all popup children; `_build_contact_panel` creates a new AlienPortrait and repeats `respond()` while `contact_reply` is nonempty. Tab changes can consequently restart the old response. `scripts/alien_portrait.gd` currently supplies texture bobbing, blink/gaze and a procedural mouth. This is scaffolding, not authored acting. `expedition_diplomacy.gd::greeting` distinguishes attitude but not first versus repeat encounter.
