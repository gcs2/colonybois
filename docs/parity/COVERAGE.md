...
- **The recovered pulse ward (old shroud) remains a damage reducer.** A separate purchased timed Shield now blocks flagship damage; Rally Call boosts actual personal/allied fire. Neither provides invisibility; do not count Cloaking Device as implemented. See [SHIP_SUPPORT.md](../SHIP_SUPPORT.md) for scenario values and remaining reference uncertainty.
