...
| Independent visual critic | Review a material mock before build and the integrated runtime after it, when presentation needs it | Spore evidence, current approved direction, proposed mock, then matched actual runtime evidence | Specific mock/runtime discrepancies and bounded refinements; any pass is limited to inspected states |
...
Each player-facing stream should connect behavior to visual communication: controls, state feedback, world assets, animation and particles where relevant. Use the task board's feature owner and source evidence; do not trade a readable, working loop for a larger feature count. For a meaningful visual gap, first compare the existing runtime with the Spore reference and approved direction. If the mock is missing, create a coordinated mock for the affected states and have the independent critic recommend approval or concrete refinements before implementation. After integration, compare the actual runtime against the approved mock at the same view, state and resolution, then request one bounded critic review. The user's recorded decisions outrank critic preference.

Loop the user in on material direction choices, conflicting evidence, paid terms and changes that could invalidate an established preference. Send a concise recommendation with the smallest useful set of options, continue independent work while waiting, and avoid blocking the whole goal on an answer that only affects one reversible decision. Record the assumption and make the reversible path easy to change. Never make a purchase or accept paid terms without explicit approval.

Each worker returns a compact handoff: task ID; branch/worktree and baseline-to-commit hashes; player behavior changed; evidence; exact checks and elapsed times; integration concerns; remaining gaps; and token/cost figures only when exposed by the runtime. Sol returns one concise summary across the swarm. For material presentation work, resolve the mock against evidence before build, then integrate and request one independent visual review of matched runtime states. Review animation, particles, transitions and sound when they carry meaning, not just static layout. A critic's review does not replace human acceptance, and a critic cannot overrule the user.
