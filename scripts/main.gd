...
func _process(delta: float) -> void:
	...
	if view == "colony":
		...
		focus.x = clampf(focus.x,-24,24)
		focus.z = clampf(focus.z,-24,24)
	if Input.is_physical_key_pressed(KEY_Q): orbit -= delta * 0.8
	if Input.is_physical_key_pressed(KEY_E): orbit += delta * 0.8
	_update_camera(delta)
	if view == "planet" and is_instance_valid(overview_globe): overview_globe.rotate_y(delta*0.06)
...
func _update_camera(delta: float = 1.0) -> void:
	...
	var elevation: float = 0.85
	if view == "galaxy": desired_focus = Vector3(1,0,6); distance = 65; elevation = 1.15
	elif view == "planet": desired_focus = Vector3.ZERO; distance = 35; elevation = 0.28
	var desired: Vector3 = desired_focus + Vector3(sin(orbit)*distance, distance*elevation,cos(orbit)*distance)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		if event.pressed and event.button_index == MOUSE_BUTTON_WHEEL_UP: zoom = maxf(12,zoom-2)
		if event.pressed and event.button_index == MOUSE_BUTTON_WHEEL_DOWN: zoom = minf(90,zoom+2)
...
func _command(action: String, args: Dictionary = {}) -> void:
	...
	if not error.is_empty(): _toast(error)
		...
		_refresh_ui()
		if action in ["colonize","discover","cheat"]: _rebuild_world()
...
func _style(bg: Color, border: Color = Color("263d50"), radius: int = 10) -> StyleBoxFlat:
...
func _button(text: String, callback: Callable, parent: Control, toggle: bool = false) -> Button:
	...
	button.add_theme_stylebox_override("normal",_style(Color("172a3b")))
	button.add_theme_stylebox_override("hover",_style(Color("244156"),MINT))
	button.add_theme_stylebox_override("pressed",_style(Color("214b49"),MINT))
...
func _panel(parent: Control, left: float, top: float, right: float, bottom: float) -> PanelContainer:
	...
	panel.add_theme_stylebox_override("panel",_style(Color("101f30f5")))
...
func _make_ui() -> void:
	...
	root.theme = theme
	var top: PanelContainer = _panel(root,18,16,1582,92)
	var bar := HBoxContainer.new()
	bar.add_theme_constant_override("separation",22)
	top.add_child(bar)
	var brand := VBoxContainer.new()
	brand.custom_minimum_size.x = 280
	bar.add_child(brand)
	brand.add_child(_label("F R O N T I E R   W O R L D S",19,WHITE))
	...
	var left_panel: PanelContainer = _panel(root,18,108,273,796)
	...
	var right_panel: PanelContainer = _panel(root,1287,108,1582,796)
	...
	center.position = Vector2(300,122)
	center.mouse_filter = Control.MOUSE_FILTER_IGNORE
	root.add_child(center)
	title_label = _label("",32)
	center.add_child(title_label)
	subtitle_label = _label("",15,MINT)
	center.add_child(subtitle_label)
	var footer: PanelContainer = _panel(root,18,812,1582,883)
	...
	toast_label.position = Vector2(300,746)
	toast_label.size = Vector2(950,55)
...
func _refresh_right() -> void:
	...
	if view == "colony":
		...
		if not surveyed:
			...
			elif str(planet.owner).is_empty(): _button("Establish colony · 180 Marks",_command.bind("colonize",{"planet":planet_id}),right_box)
...
func _draw_colony() -> void:
	...
	_box(world,Vector3(0,-0.65,0),Vector3(64,1,64),base.darkened(0.6))
...
func _draw_galaxy() -> void:
	...
	_sphere(ship_mesh,Vector3(0,0,0.7),0.18,MINT,1.2)
	_position_ship()
...
func _draw_planet() -> void:
	...
	if sim.state.colonies.has(planet_id):
		_sphere(world,Vector3(1.5,1.5,5.8),0.14,GOLD,1.0)
		_world_label(world,"COLONY SITE",Vector3(1.5,2.2,6),GOLD,20)
