...
func _colonize(pid: String) -> String:
	...
	if not str(planet.owner).is_empty(): return "This planet is already claimed."
	if state.colonies.size() >= 3: return "Prototype limit: three colonies."
	if state.credits < 180: return "A colony expedition costs 180 credits."
	state.credits -= 180
	_create_colony(pid)
	for faction: Dictionary in state.factions:
...
func _diplomacy(args: Dictionary) -> String:
	...
	if pact == "reconcile":
		...
		if state.credits < 80: return "Reconciliation costs 80 credits."
		state.credits -= 80
		faction.relation = 0
...
func _terraform(pid: String) -> String:
	...
	if state.credits < 120: return "Starting the project costs 120 credits."
	state.credits -= 120
	planet.project = true
...
func _cheat(args: Dictionary) -> String:
	...
	_log("Sandbox · " + ability.replace("_"," "))
	return ""
